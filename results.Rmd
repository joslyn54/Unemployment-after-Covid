...
generate_table <- function(year_1, year_2){
  ...
  cnt <- 2
  df <- html_table(read_html(paste(year_1, "10.html", sep="")))[3]

  repeat {
  if (cnt <= 3) {
    filename <- paste(year_1, month[cnt], ".html", sep="")
  }
  if (cnt >3 && cnt <= 12) {
    filename <- paste(year_2, month[cnt], ".html", sep="")
  }
  if (cnt > 12) {
    break
  }
...
unemploy_data <- read_excel("unemploy.xlsx")
...
# read the GDP data
gdp_data <- read.csv(file = 'GDP.csv')
